"use strict";var __createBinding=(this&&this.__createBinding)||(Object.create?(function(o,m,k,k2){if(k2===undefined)k2=k;var desc=Object.getOwnPropertyDescriptor(m,k);if(!desc||("get"in desc?!m.__esModule:desc.writable||desc.configurable)){desc={enumerable:true,get:function(){return m[k];}};}
...
if(op[0]&5)throw op[1];return{value:op[0]?op[1]:void 0,done:true};}};Object.defineProperty(exports,"__esModule",{value:true});exports.restore=void 0;var firestore_1=require("firebase-admin/firestore");var fs=__importStar(require("fs"));var uuid_1=require("uuid");var helper_1=require("./helper");var restore=function(fileName,options){var db=(0,firestore_1.getFirestore)();return new Promise(function(resolve,reject){if(typeof fileName==='object'){var dataObj=fileName;updateCollection(db,dataObj,options).then(function(){resolve({status:true,message:'Collection successfully imported!',});}).catch(function(error){reject({status:false,message:error.message});});}
else{fs.readFile(fileName,'utf8',function(err,data){if(err){console.log(err);reject({status:false,message:err.message});}
var dataObj=JSON.parse(data);updateCollection(db,dataObj,options).then(function(){resolve({status:true,message:'Collection successfully imported!',});}).catch(function(error){reject({status:false,message:error.message});});});}});};exports.restore=restore;var updateCollection=function(db,dataObj,options){if(options===void 0){options={};}
...
_f.push(_g);_h=0;_j.label=8;case 8:if(!(_h<_f.length))return[3,11];subIndex=_f[_h];revivedSubCollection={};subCollectionPath="".concat(collectionName,"/").concat(docId,"/").concat(subIndex);revivedSubCollection[subCollectionPath]=subCollections[subIndex];return[4,updateCollection(db,revivedSubCollection,options)];case 9:_j.sent();_j.label=10;case 10:_h++;return[3,8];case 11:_e++;return[3,2];case 12:_i++;return[3,1];case 13:return[2];}});});};var startUpdating=function(db,collectionName,docId,data,options){if(options.dates&&options.dates.length>0){options.dates.forEach(function(date){if(data.hasOwnProperty(date)){if(Array.isArray(data[date])){data[date]=data[date].map(function(d){return(0,helper_1.makeTime)(d);});}
else{data[date]=(0,helper_1.makeTime)(data[date]);}}
if(date.indexOf('.')>-1){(0,helper_1.traverseObjects)(data,function(value){if(!value.hasOwnProperty('_seconds')){return null;}
return(0,helper_1.makeTime)(value);});}});}
if(options.autoParseDates){(0,helper_1.parseAndConvertDates)(data);}
...
if(options.geos&&options.geos.length>0){options.geos.forEach(function(geo){if(data.hasOwnProperty(geo)){if(Array.isArray(data[geo])){data[geo]=data[geo].map(function(geoValues){return(0,helper_1.makeGeoPoint)(geoValues);});}
else{data[geo]=(0,helper_1.makeGeoPoint)(data[geo]);}}
if(geo.indexOf('.')>-1){(0,helper_1.traverseObjects)(data,function(value){if(!value.hasOwnProperty('_latitude')){return null;}
return(0,helper_1.makeGeoPoint)(value);});}});}
if(options.autoParseGeos){(0,helper_1.parseAndConvertGeos)(data);}
