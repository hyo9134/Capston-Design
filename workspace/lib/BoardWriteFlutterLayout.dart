...
class BoardWriteFlutterLayout extends StatefulWidget {

  BoardWriteFlutterLayout(
  ...
  @override
  State<BoardWriteFlutterLayout> createState() => _MyBoardWriteFlutterLayout(boardName,colName,userEmail);
}

class _MyBoardWriteFlutterLayout extends State<BoardWriteFlutterLayout> {
...
// 필드명
  ...
  _MyBoardWriteFlutterLayout(this.boardName,this.colName,this.userEmail);
...
// 문서 갱신 (Update)
  ...
  Future _uploadImage() async {
    //문서개수 불러오기
    var docCount = await countDocuments();
  ...
  //투표 업로드
  Future _uploadVote(String vote1, String vote2, String? vote3, String? vote4) async{
    var docCount = await countDocuments() -1;
              ...
              child: IconButton(
                ...
                onPressed: () async{
                  ...
                  {createDoc(_newTitleCon.text, _newContentCon.text, documentCount, now, 0,
                      0, "Y", userEmail,colName,boardName, _pickedImgs.length, 0);
                  _uploadVote(_newVote1Con.text,_newVote2Con.text,_newVote3Con.text,_newVote4Con.text,);
                  }
                  else if(!_Votevisibility)
                  {createDoc(_newTitleCon.text, _newContentCon.text, documentCount, now, 0,
                      0,  "N", userEmail,colName,boardName,_pickedImgs.length, 0);}
